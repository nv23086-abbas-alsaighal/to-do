...
def api_get_tasks():
    ...
    if tag:
        ...
        if target:
            needle = target[0]
            tasks = [
                t for t in tasks if needle in normalize_tags(t.get("tags", []))
            ]
    if sort == "due":
...
def api_task_stats():
    ...
    overdue = len(
        [
            ...
            for t in tasks
            if t.get("due_date")
            and t.get("due_date") < str(__import__("datetime").date.today())
...
def api_get_tags():
    tasks = load_tasks()
    tags = sorted(
        {tag for task in tasks for tag in normalize_tags(task.get("tags", []))}
    )
    return jsonify(tags)
...
def api_remove_tag(task_id, tag_name):
    ...
    for task in tasks:
        if task["id"] == task_id:
            task["tags"] = [
                tag
                for tag in normalize_tags(task.get("tags", []))
                if tag != target
            ]
            save_tasks(tasks)
            return jsonify(task)
...
def api_task_comments(task_id):
    ...
    for task in tasks:
        if task["id"] == task_id:
            ...
            if len(body_raw) > 500:
                error = {
                    "error": "Comment body must be 500 characters or less"
                }
                return jsonify(error), 400

            comments = task.get("comments", [])
            next_comment_id = max(
                [c.get("id", 0) for c in comments], default=0
            ) + 1
